...
CREATE VIEW
    all_doctors_view AS
SELECT
    d.full_name AS nombre_completo,
    d.age AS edad,
    d.sex AS sexo,
    m_s.name AS especialidad_medica
FROM
    doctor d
    LEFT JOIN medical_speciality m_s ON (d.id_speciality = m_s.id_speciality);

CREATE VIEW
    male_doctors_view AS
SELECT
    d.full_name AS nombre_completo,
    d.age AS edad,
    d.sex AS sexo,
...
CREATE VIEW
    patients_per_room_view AS
SELECT
    r.name AS nombre_sala,
    COUNT(DISTINCT id_assigned_room) AS cantidad_pacientes
FROM
    patient p
    LEFT JOIN room r ON (p.id_assigned_room = r.id_room)
GROUP BY
...
CREATE VIEW
    nurses_view AS
SELECT
    n.full_name AS nombre,
    n.age AS edad,
    n.sex AS sexo,
    r.name AS sala_asignada
FROM
    nurse n
    LEFT JOIN room r ON (n.id_assigned_room = r.id_room);

CREATE VIEW
    patients_diagnosis_view AS
SELECT
    p.full_name AS nombre_paciente,
    ds.name AS diagnostico,
    pd.date AS fecha_diagnostico
FROM
...
SELECT
    p.full_name AS nombre_paciente,
    d.full_name AS nombre_medico_asignado,
    m.med_name AS nombre_medicina_recetada,
    mt.name AS estudio_medico
FROM
    turn t
